...
<head>
    ...
    <meta name="keywords" content="db, pruebas, usabilidad" />
...
class BaseDatos
{
    ...
    public function __construct()
    {
        ...
        if (count($_POST) > 0) {
            ...
            if (isset($_POST['crear_tabla']))
                $this->crear_tabla();
            if (isset($_POST['insertar_en_tabla']))
                $this->insertar_en_tabla_gui();
            if (isset($_POST['buscar_en_tabla']))
                $this->buscar_en_tabla_gui();
            if (isset($_POST['modificar_en_tabla']))
                $this->modificar_en_tabla_gui();
            if (isset($_POST['eliminar_en_tabla']))
                $this->eliminar_en_tabla_gui();
            if (isset($_POST['generar_informe']))
                $this->generar_informe();
            if (isset($_POST['importar_csv']))
                $this->importar_csv_gui();
            if (isset($_POST['exportar_csv']))
                $this->exportar_csv();

            if (isset($_POST['insertar']))
                $this->insertar_en_tabla();
            if (isset($_POST['buscar']))
                $this->buscar_en_tabla();
            if (isset($_POST['modificar']))
                $this->modificar_en_tabla();
            if (isset($_POST['eliminar']))
                $this->eliminar_en_tabla();
    ...
    private function crear_db()
    {
        ...
        if ($this->db->query($create_db) === TRUE)
            $this->mensaje_de_exito(
                "Base de datos " . $this->db_name . " creada con éxito"
            );
        else
    ...
    private function conectarse_db()
    {
        ...
        if ($this->db->connect_error) // si detectamos algún tipo de error
            ...
            $this->mensaje_de_exito(
                'Conexión establecida con éxito con el HOST ' . $this->db->host_info
            );
    }
    ...
    private function crear_tabla()
    {
        ...
        if ($this->db->query($tabla) === TRUE)
            $this->mensaje_de_exito(
                "La tabla PruebasUsabilidad se ha creado correctamente"
            );
        else // En caso de que no lo hayamos conseguido...
            $this->mensaje_de_error(
                "ERROR durante la creación de la tabla: ",
                $this->db->error
            );
    ...
    private function insertar_en_tabla()
    {
        ...
        if (
            ...
            $this->mensaje_de_error(
                ...
                "error en la validación de los datos, ningún campo puede estar vacío"
            );
        else {
            try {
    ...
    private function buscar_en_tabla()
    {
        ...
        try {
            ...
            if ($resultado->fetch_assoc() != NULL) {
                ...
                echo "
                 <h3> Los datos encontrados son los siguientes: </h3>
                 <ul>
                     <li> DNI: $id </li>
                     <li> Nombre: $nombre </li>
                ...
                $this->mensaje_de_error(
                    "ERROR: ",
                    "no se han encontrado datos para la búsqueda solicitada"
                );
        ...
        echo "
         <form action='#' method='post'>
             ...
             <fieldset>
                 <legend> ¿Sexo? </legend>
                 <label for='insertar_sexo_hombre'>Hombre</label>
                 <input type='radio' id='insertar_sexo_hombre' name='modificar_sexo' value='0'>
                 
                 <label for='insertar_sexo_mujer'>Mujer</label>
                 <input type='radio' id='insertar_sexo_mujer'  name='modificar_sexo' value='1'>
             </fieldset>
             ...
             <fieldset>
                 <legend> Se ha relizado correctamente la tarea? </legend>
                 <label for='insertar_es_completada_si'>Sí</label>
                 <input type='radio' id='insertar_es_completada_si' name='modificar_es_completada' value='1'>
                 <label for='insertar_es_completada_no'>No</label>
                 <input type='radio' id='insertar_es_completada_no' name='modificar_es_completada' value='0'>
             </fieldset>
    ...
    private function modificar_en_tabla()
    {
        ...
        $sexo = $_POST['modificar_sexo'] === '1' ? true : false;
        $nivel = $_POST['modificar_nivel'];
        $tiempo = $_POST['modificar_tiempo'];
        $es_completada = $_POST['modificar_es_completada'] === '1' ? true : false;
    ...
    private function importar_csv()
    {
        ...
        try {
            ...
            while (($datos = fgetcsv($archivo, 1000, ";")) !== false) {
                ...
                $is_inserted = $query->bind_param(
                    'sssssiiiiissi', $datos[0],
                    $datos[1],
                    $datos[2],
                    $datos[3],
                    $datos[4],
