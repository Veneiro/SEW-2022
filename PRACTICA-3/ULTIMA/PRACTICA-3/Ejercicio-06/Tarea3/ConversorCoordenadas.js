class Conversor{
	...
	cmdLat2UTM_click(op1, op2, stack){
        var xy = new Array(2);
        var lon = parseFloat (op1);

        if ((lon < -180.0) || (180.0 <= lon)) {;
            return false;
        }

        var lat = parseFloat (op2);
        ...
        zone = this.operations.LatLonToUTMXY (this.operations.DegToRad (lat), this.operations.DegToRad (lon), zone, xy);
	...
	cmdUTM2Lat_click(op1, op2, stack, area){
        ...
        this.operations.UTMXYToLatLon (x, y, zone, southhemi, latlon);

        stack.push(this.operations.RadToDeg (latlon[1]));

        stack.push(this.operations.RadToDeg (latlon[0]));
